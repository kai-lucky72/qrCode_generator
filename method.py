...
def colorcode(text, color='', bg=''):
   ...
   return f"{bg}{color}{text}{COLORS_WITH_CODES['clear']}"
...
def listenForKeys(key):
   ...
   if len(key.rsplit(" ")) <= 1:
      print(colorcode("Unkown command", bg="bg-red"))
      return
...
def test_push(ct, dir,interval,branch):
...
def printcommands():
   print(colorcode("q - quit\tp - pause or resume\tcb - change branch", "yellow"))
